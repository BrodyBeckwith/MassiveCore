...
public class InventoryUtil
{
	...
	public static boolean isRepairable(Material material)
	{
		...
		if (material == Material.TROPICAL_FISH) return false;
		...
		if (material == Material.ELDER_GUARDIAN_SPAWN_EGG) return false;
	...
	public static Inventory clone(Inventory inventory, boolean playerSupport)
	{
		...
		else
		{
			...
			if (inventory instanceof CraftInventoryCustom)
			{
				try
				{
					Class<?> clazz= Class.forName("org.bukkit.craftbukkit.v1_14_R1.inventory.CraftInventoryCustom$MinecraftInventory");
					Method method = clazz.getDeclaredMethod("getTitle");
					method.setAccessible(true);
					Object object = clazz.cast(((CraftInventoryCustom) inventory).getInventory());
					title = (String) method.invoke(object);
				}
